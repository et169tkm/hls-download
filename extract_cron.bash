...
[[ "${arg_p:-}" ]]     || help      "Programme name is required"
...
if [ "$arg_p" = "sunnyday" ]; then
    channel=cr2
    nowdate=`date +%F`
    start_time=`date --date="$nowdate 08:00:00" +%s`
    duration=7500
    out_file_name=${arg_p}'_'$nowdate'.ts'
elif [ "$arg_p" = "jobawong" ]; then
    channel=cr2
    nowdate=`date +%F`
    start_time=`date --date="$nowdate 10:00:00" +%s`
    duration=7500
    out_file_name=${arg_p}'_'$nowdate'.ts'
elif [ "$arg_p" = "ning" ]; then
    channel=cr2
    nowdate=`date +%F`
    start_time=`date --date="$nowdate 17:00:00" +%s`
    duration=3900
    out_file_name=${arg_p}'_'$nowdate'.ts'
elif [ "$arg_p" = "ning_movie" ]; then
    channel=cr2
    nowdate=`date +%F`
    start_time=`date --date="$nowdate 13:00:00" +%s`
    duration=7500
    out_file_name=${arg_p}'_'$nowdate'.ts'
elif [ "$arg_p" = "test" ]; then
    channel=cr2
    nowdate=`date +%F`
    start_time=`date --date="$nowdate 08:00:00" +%s`
    duration=7500
    #out_file_name=${arg_p}'_'$nowdate'.ts'
    out_file_name=${arg_p}'_'$nowdate'.mp4'
else
    error "Unknown programme: $arg_p"
    exit 1
fi
